...
public class PPFXWindow : EditorWindow
{
    ...
    void Update()
    {
        if (!Application.isPlaying && pfxPreview != null)
        {
            ...
            if (pfxPreview.width != position.width || pfxPreview.height != pfxPreviewHeight)
                pfxPreview = new RenderTexture(
                    (int)position.width,
                    pfxPreviewHeight,
                    (int)RenderTextureFormat.ARGB32
            ...
            {
                pfxParentContainer.transform.Rotate(
                    Vector3.up * Time.deltaTime * 100000f,
                    Space.Self
                );
            }
        }
    ...
    static void LoadPrefabs()
    {
        ...
        for (int i = 0; i < pfxFolders.Length; i++)
        {
            string _path = Path.Combine(
                Path.Combine(Application.dataPath, pfxAssetPath),
                pfxFolders[i]
            );
            //Debug.Log(_path);
            ...
            if (fileInfo.Length != pfxAssetCount[i])
            {
                ...
                foreach (FileInfo file in fileInfo)
                {
                    ...
                    string _assetPath = Path.Combine(
                        Path.Combine(pfxAssetPath, pfxFolders[i]),
                        _assetName
                    );
                    ...
                    if (_extension == ".prefab")
                    {
                        ...
                        Texture _img = (Texture)
                            AssetDatabase.LoadAssetAtPath(_imgPath, typeof(Texture));
    ...
    void PreviewGUI()
    {
        if (!Application.isPlaying && pfxPreview != null)
        {
            ...
            EditorGUI.DrawPreviewTexture(
                new Rect(0, 0, position.width, pfxPreviewHeight),
                pfxPreview
            );
            EditorGUILayout.EndHorizontal();
        }
    ...
    void LibraryGUI()
    {
        ...
        ////////////////////////////
        GUILayout.BeginArea(
            new Rect(0, pfxPreviewHeight, position.width, position.height - pfxPreviewHeight)
        );
        ...
        if (pfxPreviewContainer != null)
        {
            ...
            if (GUILayout.Button("Add Prefab", GUILayout.Height(40)))
            {
                GameObject _inst =
                    Instantiate(
                        pfxPrefabList[selFolderIndex][selIndex],
                        Vector3.zero,
        ...
        {
            pfxListToggle[f] = GUILayout.Toggle(
                pfxListToggle[f],
                (pfxListToggle[f] ? "close " + pfxFolders[f] : "open " + pfxFolders[f]),
            ...
            {
                int _rows = 0;
                pfxGUIScrollPos[f] = EditorGUILayout.BeginScrollView(
                    pfxGUIScrollPos[f],
                    GUILayout.Height(_rows * pfxGUIButtonHeight)
                );
                ...
                for (int x = 0; x < pfxPrefabList[f].Length; x++)
                {
                    ...
                    if (
                        GUILayout.Button(
                            _tex,
                            GUILayout.Width(pfxGUIButtonWidth),
                            GUILayout.Height(pfxGUIButtonHeight)
                        )
                    )
                    {
    ...
    void SetPreviewImage()
    {
        ...
        _previewImg.ReadPixels(
            new Rect((position.width - 200) / 2, 0, pfxPreview.width, 200),
            0,
            0
        );
        _previewImg.Apply();
